...
@IonicPage()
@Component({
  selector: 'page-munster-chat-room',
  templateUrl: 'munster-chat-room.html',
})
export class MunsterChatRoomPage {
  user:string = ''
  message:string = ''
  messages: object[];
  currentUser: User;
  ...
  constructor(public navParam: NavParams,public db: AngularFireDatabase) {
    firebase.auth().onAuthStateChanged(user => {
      if (user) {
        ...
        this.db.list('/chat/Munster').valueChanges().subscribe(data => {
          this.messages = data
        });
      }
    })
  ...
  sendMessage(){
    // alert(this.user);
    this.db.list('/chat/Munster').push({
      userName: this.FName,
