...
@IonicPage()
@Component({
  selector: 'page-leinster-chat-room',
  templateUrl: 'leinster-chat-room.html',
})
export class LeinsterChatRoomPage {
  ...
  constructor(public navParam: NavParams,public db: AngularFireDatabase) {
    firebase.auth().onAuthStateChanged(user => {
      if (user) {
        ...
        this.db.list('/chat/Leinster').valueChanges().subscribe(data => {
          this.messages = data
        });
      }
    })
  ...
  sendMessage(){
    // alert(this.user);
    this.db.list('/chat/Leinster').push({
      userName: this.FName,
