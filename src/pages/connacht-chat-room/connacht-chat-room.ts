...
@IonicPage()
@Component({
  selector: 'page-connacht-chat-room',
  templateUrl: 'connacht-chat-room.html',
})
export class ConnachtChatRoomPage {
  ...
  constructor(public navParam: NavParams,public db: AngularFireDatabase) {
    firebase.auth().onAuthStateChanged(user => {
      if (user) {
        ...
        this.db.list('/chat/Connacht').valueChanges().subscribe(data => {
          this.messages = data
        });
      }
    })
  ...
  sendMessage(){
    // alert(this.user);
    this.db.list('/chat/Connacht').push({
      userName: this.FName,
