...
@IonicPage()
@Component({
  selector: 'page-ulster-chat-room',
  templateUrl: 'ulster-chat-room.html',
})
export class UlsterChatRoomPage {
  user:string = ''
  message:string = ''
  messages: object[];
  currentUser: User;
  ...
  constructor(public navParam: NavParams,public db: AngularFireDatabase) {
    firebase.auth().onAuthStateChanged(user => {
      if (user) {
        ...
        this.db.list('/chat/Ulster').valueChanges().subscribe(data => {
          this.messages = data
        });
      }
    })
